...
def calc():
    store = {"red": 12, "blue": 14, "green": 13}
    input_list = input.split("\n")
    total = 0
    for curr_line in input_list:
        # print(curr_line)
        game, rounds = curr_line.split(":")
        game_idx = game[5:]
        rounds = rounds.split(";")
        balls_here = {'red': 0, 'blue': 0, 'green': 0}
        maxes = 1
        for round in rounds:
            balls = round.split(',')
            for ball in balls:
                cnt, color = ball[1:].split(' ')
                balls_here[color] = max(int(cnt), balls_here[color])
        total += balls_here['red']*balls_here['blue']*balls_here['green']

        print(total, game_idx)
        # break
    return total
